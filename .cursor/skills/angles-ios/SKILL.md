...
Follow root `BUILD.md` (order + status). Update it when adding a screen. MVVM starts with the first real screen. No SwiftData. Home matches Profile pager chrome: **All** (default, mixed covers) plus four style tabs on one row with the filter icon, style-tinted glass header, tab-bar footer fade, Apply-only Life areas + Moods sheet. Settings lives on Profile only. One paged `GET /feed`; style tabs filter in memory and open on that angle, same as Profile. Profile is identity (JM avatar, session name until Auth) plus five expanding tabs: Favorites first, then the four styles. Style tabs keep owned cards that have that angle and open on it; there is no All tab on Profile. Both tabs share `StyleTabPager.swift` for chips and wash scrubbing.
